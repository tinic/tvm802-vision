...
| `fiducial/` | Down-vision `frame_NNNN.png` from a real placement, used by `CheckMark` / `CheckMark2` / `CheckTemplate`. | Currently SKIPPED by `smoke_comp.sh` (the component smoke uses `detect_component` only). Kept here for a future fiducial smoke rig. |
