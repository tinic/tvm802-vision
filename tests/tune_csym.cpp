// Offline circular-symmetry tuning rig (NOT committed; gitignored). Runs the real
// vis::detect_circular_symmetry over captured PNG fiducial frames with the
// production config and reports per-frame results + aggregate hit-rate and centroid
// stability. Build standalone:
//   g++ -std=c++20 -O2 -I src tests/tune_csym.cpp \
...
int main(int argc, char** argv) {
    const double refX = 370.0, refY = 270.0;  // mvo -50/-30 -> ref 370/270
    const int searchR = 45;                   // areaMin=45 -> Range search radius
