...
public:
    ...
    // accurate score and center. median: combine rings by MEDIAN per-ring variance
    // instead of the area-weighted mean -- robust to a few glare-corrupted rings.
    double score(const cv::Mat& g, double cx, double cy, double& edgeR, int step = 1,
                 bool median = false) const;
